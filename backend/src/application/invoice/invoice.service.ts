...
import { FraudService } from '../../domain/fraud/fraud.service';
import { RailService } from '../../domain/rail/rail.service';
import { CostEstimatorService } from '../../domain/rail/cost-estimator.service';
import { FxService } from '../../infrastructure/fx/fx.service';
import { AiService } from '../../infrastructure/ai/ai.service';
import { RailName } from '../../domain/rail/rail.types';
...
export class InvoiceService {
  ...
  async analyze(dto: AnalyzeInvoiceDto) {
    ...
    const datePart = toCompactTimestamp(analyzedAt);
  ...
  async extract(dto: ExtractInvoiceDto): Promise<Partial<AnalyzeInvoiceDto>> {
    ...
    this.logger.log(`✓ extract done  inv=${result.invoice_number ?? 'unknown'}`);
    return result;
  }
