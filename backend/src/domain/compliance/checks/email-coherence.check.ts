...
// DATA: consider moving GENERIC_PROVIDERS to data/generic-email-providers.json for easier updates

@Injectable()
export class EmailCoherenceCheck {
  private readonly logger = new Logger(EmailCoherenceCheck.name);
  ...
  async run(dto: AnalyzeInvoiceDto): Promise<ComplianceCheckResult> {
    ...
    if (abstractApiKey) {
      try {
        ...
        // Fail-safe: AbstractAPI unavailable — continue with rule-based checks only
        this.logger.debug(
          `AbstractAPI email validation unavailable — falling back to rule-based: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    }
