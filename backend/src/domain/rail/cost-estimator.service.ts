...
// DATA: consider moving to data/rail-fees.json for runtime updatability without redeploy
const RAIL_FEES: Record<RailName, FeeSpec> = {
  ...
  CHATS_RTGS: { flat_fee_hkd: [0, 200],   fx_markup_pct: 0, intermediary_usd: [0, 0],   settlement_time: 'Same day (business hours)' },
...
export class CostEstimatorService {
  ...
  async estimate(amountHkd: number, rails: RailName[]): Promise<CostEstimate[]> {
    const usdToHkd = await this.fxService.getRate('USD', 'HKD').catch(() => THRESHOLDS.FX_USD_HKD_FALLBACK);
