...
import type { RiskReason, RiskScore } from './risk-score.types';
import { getRiskLevel, type RiskLevel } from '../shared/utils/risk.utils';
...
export class RiskScoreService {
  calculateRiskScore(invoice: Invoice): RiskScore {
    ...
    if (invoice.risk_signals.payment_details_changed) {
      score += 35;
      reasons.push({ label: 'Payment details changed', severity: 'CRITICAL' });
    }

    if (invoice.risk_signals.urgency_language) {
      score += THRESHOLDS.FRAUD_URGENCY_POINTS;
      reasons.push({ label: 'Urgency language detected', severity: 'HIGH' });
    }

    if (invoice.risk_signals.pressure_language) {
      score += 15;
      reasons.push({ label: 'Pressure language detected', severity: 'HIGH' });
    }

    if (invoice.risk_signals.secrecy_language) {
      score += THRESHOLDS.FRAUD_SECRECY_POINTS;
      reasons.push({ label: 'Secrecy language detected', severity: 'HIGH' });
    }

    if (!namesLikelyMatch(invoice.payment.beneficiary_name, invoice.supplier.name)) {
      score += 25;
      reasons.push({ label: 'Beneficiary does not match supplier', severity: 'HIGH' });
    }
    ...
    if (
      ...
      score += 10;
      reasons.push({ label: 'Unknown stablecoin network', severity: 'MEDIUM' });
    }

    if (isUnknownValue(invoice.supplier.name) || isUnknownValue(invoice.supplier.country)) {
      score += 10;
      reasons.push({ label: 'Supplier identity incomplete', severity: 'MEDIUM' });
    }

    score = Math.min(score, THRESHOLDS.RISK_MAX_SCORE);
...
function normalizeName(name: string): string {
  return name
    ...
    .replace(/\bs\.?\s*l\.?\b/g, '')
    .replace(/\b(?:aps|gmbh|limited|ltd|ltd\.|inc|corp|company)\b/g, '')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim();
}
...
function buildRiskSummary(level: RiskLevel, reasons: RiskReason[]): string {
  if (reasons.length === 0) return 'Low fraud risk. No risk signals were detected.';
...
function reasonLabelToSummaryClause(label: string): string {
  ...
  if (lowerLabel.endsWith(' detected')) return `${lowerLabel.replace(/ detected$/, '')} was detected`;
  return lowerLabel;
}
