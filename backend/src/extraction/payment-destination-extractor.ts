...
export function extractPaymentDestination(
  ...
  if (requestedMethod === 'CHATS_RTGS') {
    return {
      ...
      bank_code:
        extractCode(
          findLabeledValue(lines, ['Bank Code', 'HK Bank Code']),
          /([0-9]{2,12})/,
        ) ||
        firstRegexGroup(
          paymentText,
          /\bbank code\s*[:#-]?\s*([0-9]{2,12})\b/i,
        ) ||
        undefined,
      branch_code:
        extractCode(
          findLabeledValue(lines, ['Branch Code', 'HK Branch Code']),
          /Branch Code\s*:\s*([0-9]{2,12})/i,
        ) ||
        extractCode(
          findLabeledValue(lines, ['Branch Code', 'HK Branch Code']),
          /^([0-9]{2,12})$/,
  ...
  if (requestedMethod === 'CIPS') {
    return {
      ...
      cnaps_code:
        ...
        firstRegexGroup(
          paymentText,
          /\b(?:cnaps|cips)\s*(?:routing|code|id)?\s*[:#-]?\s*([A-Z0-9]{6,20})\b/i,
        ) ||
        undefined,
    };
  }
  ...
  return {
    raw_text: truncateRawText(paymentText || text),
  };
}
...
export function findPaymentBeneficiary(
  ...
  if (requestedMethod === 'STABLECOIN') {
    const alternativeAccount = findLabeledValue(lines, [
