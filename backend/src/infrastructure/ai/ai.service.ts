...
import { AuditInvoiceDto } from '../../application/invoice/dto/audit-invoice.dto';
import type { GoodsClassification } from '../../domain/invoice/invoice.types';

const CLASSIFY_MODEL = process.env.CLAUDE_MODEL_CLASSIFY ?? 'claude-haiku-4-5-20251001';
const EXTRACT_MODEL  = process.env.CLAUDE_MODEL_EXTRACT  ?? 'claude-sonnet-4-6';
            ...
            text: `Extract invoice fields and return ONLY valid JSON matching this schema (omit fields you cannot read):
{
  ...
  "payment": {
    ...
    if (!raw) throw new Error('No JSON found in extraction response');
    return JSON.parse(raw) as Partial<AnalyzeInvoiceDto>;
  }
  ...
  async generateAuditSummary(dto: AuditInvoiceDto): Promise<string> {
    ...
    try {
      ...
      this.logger.warn(
        'OpenRouter audit call failed — falling back to example summary',
...
function buildExampleSummary(dto: AuditInvoiceDto): string {
  const level = dto.fraud_level.toUpperCase();
  const riskLabel =
    level === 'LOW'
      ? 'low'
