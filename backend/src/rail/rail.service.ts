...
export class RailService {
  recommend(
    ...
    if (supplierCountry === 'Hong Kong' && currency === 'HKD' && dto.payment.amount < 100_000) {
      rail = 'FPS';
      reason = 'Local HKD payment under HK$100,000 — FPS is fastest and cheapest';
    } else if (supplierCountry === 'Hong Kong' && amountHkd >= 100_000) {
      rail = 'CHATS';
  ...
  private evaluateAlternative(
    ...
    switch (rail) {
      case 'FPS':
        ...
        if (amountHkd > 1_000_000) return { rail, eligible: false, reason: 'Amount exceeds FPS limit of HK$1,000,000' };
      ...
      case 'STABLECOIN':
        ...
        if (fraud.score > 30) return { rail, eligible: false, reason: `Fraud score ${fraud.score} too high for stablecoin (max 30)` };
        if (amountHkd > 500_000) return { rail, eligible: false, reason: 'Amount exceeds stablecoin policy limit of HK$500,000' };
